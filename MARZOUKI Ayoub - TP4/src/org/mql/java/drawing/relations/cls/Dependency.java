...
public class Dependency {
	...
	public static void drawAdjacent(Graphics g, int[] sourcePosition, int[] targetPosition, int direction) {
		...
		g2d.setStroke(Utilities.createDashedStroke(dashPattern, 2));
	    ...
	    if (direction == 0) {
		    Utilities.drawSimpleArrow(g2d, childX, childY, childX, childY, 4);
	    } else {
		    Utilities.drawSimpleArrow(g2d, parentX, parentY, parentX, parentY, 2);
	    }
	}
	...
	public static void draw(Graphics g, int[] sourcePosition, int[] targetPosition, int direction, int verticalLineLength) {
	    ...
	    if (direction == 0) {
		    Utilities.drawSimpleArrow(g2d, childX, childY, childX, childY, 3);
		    
	    } else {
		    Utilities.drawSimpleArrow(g2d, childX, childY, childX, childY, 1);
	    }
	}
